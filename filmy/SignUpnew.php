...
if(isset($_POST["log"])){
    ...
    if($mail&&$password){
        ...
        if($rows){
            echo "恭喜你成功登入了";
            echo "<script> window.location.href='filmy-2.html';</script>";
        }
        else{
            echo    "<script>
                    $('div input').val('');
    ...
    if(isset($_POST["send"])){
        ...
        $Password=$_POST["Password"];
        if(empty($Name)|| empty($Date)|| empty($Phone)||  empty($mail)|| empty($Account)|| empty($Password)){
            echo "有空白的欄位";
        }
        else{
            $conn=OpenCon("filmy");
            $acc="SELECT * FROM Member";
            $acc="INSERT INTO Member"."(Account,Name,Date,Phone,email,Password)"."VALUES('$Account','$Name','$Date','$Phone','$mail','$Password')";
...
<div class="container" id="container">
	<div class="form-container sign-up-container">
		<form method="post" action="">
            ...
            <input type="text" placeholder="Account" name="Account" id="Account" required>
