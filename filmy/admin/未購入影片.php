        ...
        <div style=background:#000000 class=sky>
            <div class=container-player>
                ...
                <ul class=member>
                    ...
                    <li class=top_btn_notice><span id=topBar_light_0 style=display:none></span><a href="../../complete/homePage/Homepage.html"></a>
    ...
    <nav style=background:#000000 class="mainmenu on_top">
        <div style=background:#000000 class=container-player>
            ...
            <ul>
                ...
                <li style=background:#000000><a href="../../complete/homePage/Homepage.html">首頁</a></li>
                <li style=background:#000000><a href="filmy_no_like_supervisor.php">所有影音</a></li>

<li style=background:#000000><a href= "問題回報New_supervisor.php">問題回報</a></li>
<li style=background:#000000><a href="會員管理new.php">會員管理</a></li>
<li style=background:#000000><a href="會員管理影片new.php">影片管理</a></li>
<li style=background:#FFCC6E><a href="未購入影片.php">未購入影片清單</a></li>
<li style=background:#000000><a href="../../complete/test/page.php">趨勢分析</a></li>
	...
	<div style="width:100%;text-align:center"><Form METHOD="POST" ACTION="新增未購入影片.php">
<?php
...
<style> .bt{ border: none; /* 去掉邊框 */ background: #FFCC6E; /* 改背景色 */ color: black; /* 字改成白色 */ font-size: 15px; /* 改字體大小 */ padding: 20px; /* 按鈕內邊距離 */ width: 200px; /* 按鈕寬度 */ border-radius: 5px; /* 圓角5px */ } .bt span{ transition: 0.5s; /* 過渡0.5秒 */ } .bt:hover span{ padding-right: 25px; /* 當滑鼠在按鈕上時span內邊右距離25px */ } .bt span:after{ content: "»"; /* span後面追加內容為» */ opacity: 0; /* 透明度為0 */ transition: 0.5s; /* 加個0.5秒的過渡 */ } .bt:hover span:after{ opacity: 1; /* 透明度為1，就是顯示 */ } </style>
...
<div class="container" style="text-align:center;width:900px;">
  <table bgcolor="black" width="900" border="1" class="table table-bordered table-condensed table-striped table-hover">
   ...
   <tr>
       <th><font color="white"><h2>分類</h2></font></th>
       <th><font color="white"><h2>影片名</h2></font></th>
       <th><font color="white"><h2>圖片</h2></font></th>
       <th><font color="white"><h2>投票數</h2></font></th>
       <th><font color="white"><h2>管理</h2></font></th>
              </tr>
      <?
...
$str = "SELECT * FROM Film_NotBuy WHERE FilmName LIKE '%" . $SearchResult . "%' OR FilmName LIKE '%" . $SearchResult . "%'";
               ...
               echo"fail";
                ...
                if($num>0){
                    while($row=mysqli_fetch_array($rows)){
                        ...
                        ?>
                        <tr>
                      <td><font color="white"><h2><?php echo $category;?></h2></font></td>
                      <td><font color="white"><h2><?php echo $FilmName;?></h2></font></td>
					<td><font color="white"><img src=<?php echo $photo;?>></font></td>
					
                      <td><font color="white"><h2><?php echo $vote;?></h2></font></td>
                    <td>
                        <a href='未購入影片內容編輯.php?id=<?php echo $FilmName;?>' class="btn btn-default"><h2>編輯</h2></a>
                        <a href='javascript:void(0);' class='btn btn-default del_article' data-id="<?php echo $FilmName;?>"><h2>刪除</h2></a>
      ...
      <?
              ...
              <?php if($articles):?>
                <?php foreach($articles as $article):?>
                  <tr>
                      <td><font color="white"><h2><?php echo $article['Category'];?></h2></font></td>
                      <td><font color="white"><h2><?php echo $article['FilmName'];?></h2></font></td>
					<td><font color="white"><img src=<?php echo $article['Photo'];?>></font></td>
					
                      <td><font color="white"><h2><?php echo $article['Vote'];?></h2></font></td>
                      <td><a href='未購入影片內容編輯.php?i=<?php echo $article['FilmName'];?>' class="btn btn-default"><h2>編輯</h2></a>
                          <a href='javascript:void(0);' class='btn btn-default del_article1' data-id="<?php echo $article['FilmName'];?>"><h2>刪除</h2></a>
                    </td>
                  </tr>
                <?php endforeach;?>
              <?php else:?>
                <tr>
                  <td colspan="5"><font color="white">無資料</font></td>
                </tr>
              <?php endif;?>
  <?}?>
  </table>
   <div class="row">
          ...
          <div class="col-xs-12">
         <button  class="bt" ><span>   <a href='新增未購入影片.php' class="btn btn-default"><strong>新增未購入影片</strong></button></span></a>
          </div>
          <div class="col-xs-12">
</div> 
    ...
    <script>
    	$(document).on("ready", function(){
    		...
    		//表單送出	
    		$("a.del_article1").on("click", function(){
    			//宣告變數
    			var this_tr = $(this).parent().parent();
    			
					$.ajax({
            type : "POST",
            url : "../php/del_article1.php", //因為此檔案是放在 admin 資料夾內，若要前往 php，就要回上一層 ../ 找到 php 才能進入 add_article.php
            data : {
