...
<style> .bt{ border: none; /* 去掉邊框 */ background: #FFCC6E; /* 改背景色 */ color: black; /* 字改成白色 */ font-size: 28px; /* 改字體大小 */ padding: 20px; /* 按鈕內邊距離 */ width: 200px; /* 按鈕寬度 */ border-radius: 5px; /* 圓角5px */ } .bt span{ transition: 0.5s; /* 過渡0.5秒 */ } .bt:hover span{ padding-right: 25px; /* 當滑鼠在按鈕上時span內邊右距離25px */ } .bt span:after{ content: "»"; /* span後面追加內容為» */ opacity: 0; /* 透明度為0 */ transition: 0.5s; /* 加個0.5秒的過渡 */ } .bt:hover span:after{ opacity: 1; /* 透明度為1，就是顯示 */ } </style>
          ...
          <div class="col-xs-12">
            <form id="edit_article_form">
              ...
              <div class="form-group">
                <label for="FilmName"><font color="white" size="10">影片</font> </label>
                <input type="input" class="form-control" id="FilmName" value="<?php echo $data['FilmName'];?>">
              </div>
               <div class="form-group">
                <label for="Category"><font color="white" size="10">分類</font></label>
                <select id="Category" class="form-control">
                  <option value="Taiwan" <?php echo ($data['Category'] == "Taiwan")?"checked":"";?>>Taiwan</option>
                  <option value="Animation" <?php echo ($data['Category'] == "Animation")?"checked":"";?>>Animation</option>
                    <option value="US" <?php echo ($data['Category'] == "US")?"checked":"";?>>US</option>
                  <option value="Korea" <?php echo ($data['Category'] == "Korea")?"checked":"";?>>Korea</option>
				  <option value="Japan" <?php echo ($data['Category'] == "Japan")?"checked":"";?>>Japan</option>
                  <option value="India" <?php echo ($data['Category'] == "India")?"checked":"";?>>India</option>
                </select>
              </div>
              <div class="form-group">
                <label for="Route"><font color="white" size="10">連結</font></label>
                <textarea type="input" class="form-control" id="Route"><?php echo $data['Route'];?></textarea>
              </div>
                <div class="form-group">
                <label for="supplier"><font color="white" size="10">供應商</font></label>
                <textarea type="input" class="form-control" id="supplier"><?php echo $data['Supplier'];?></textarea>
              </div>
			  <div class="form-group">
                <label for="Photo"><font color="white" size="10">圖片</label>
                <textarea type="input" class="form-control" id="Photo"><?php echo $data['Photo'];?></textarea>
              </div>
    	...
    	$(document).on("ready", function(){
    		...
    		$("#edit_article_form").on("submit", function(){
    			...
    			if($("#title").val() == '' || $("#content").val() == ''){
    				...
    				$("div.loading").html('');
					  ...
					  //使用 ajax 送出 帳密給 verify_user.php
						$.ajax({
	            type : "POST",
	            url : "../php/update_article1.php", //因為此檔案是放在 admin 資料夾內，若要前往 php，就要回上一層 ../ 找到 php 才能進入 add_article.php
	            data : {
				  
	              FilmName : $("#FilmName").val(), //使用者帳號
				  Category : $("#Category").val(), //使用者帳號
	              Supplier : $("#supplier").val(), //使用者帳號
                Route : $("#Route").val(), //使用者帳號
				  Photo:$("#Photo").val(),
	            ...
	            if(data == "yes")
	            {
	              //註冊新增成功，轉跳到登入頁面。
	              alert("更新成功，點擊確認回列表");
	              window.location.href = "未購入影片.php";
	            }
	            else
	            {
	              alert("更新錯誤"+data);
