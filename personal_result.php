...
<?php
    ...
    if(isset($_POST["send"])){
        echo"alert('Hello')";
        $newlittle=$_POST["Little"];
        $newname=$_POST["user"];
        $newdate=$_POST["D"];
        $newphone=$_POST["P"];
        $sql="UPDATE Member set LittleName='$newlittle',Name='$newname',Date='$newdate',Phone='$newphone'where email='".$mail."'";
        if(!mysqli_query($conn,$sql)){
        //echo "更新紀錄失敗...<br/>".mysqli_error($conn);
            echo "alert('bad')";
        }
    else {
...
<style id=dialogifyCss>
    ...
    .card {
        ...
        padding: 50px 40px;
        border-radius: 5px;
        -webkit-box-shadow: 0px 4px 3px rgba(106,131,134,0.1);
    ...
    .form__text {
        ...
        .form__text p.style-inline {
            display: inline-block;
            margin-left: 20px
        }

        .form__text p.style-block {
            display: block;
            position: relative;
            margin-top: 13px;
            margin-left: 5px
    ...
    .form__status {
        ...
        .form__status.is-warning {
            ...
            .form__status.is-warning:before {
                content: "\e000"
            }
    ...
    .form__multiple {
        ...
        .form__multiple label {
            font-size: 17px;
            color: #9b9b9b
        }

    .form__multiple__input {
        position: relative;
        display: block;
        margin-top: 10px
    }

        .form__multiple__input input {
            padding: 10px 5px;
            background: none;
            border: none;
            width: 265px;
            ...
            .form__multiple__input input.is-new {
                width: 110px;
                cursor: pointer
            }

                .form__multiple__input input.is-new::-webkit-input-placeholder {
                    color: #699ecc
                }

                .form__multiple__input input.is-new::placeholder {
                    color: #699ecc
                }

            .form__multiple__input input.is-text {
                padding: 0px 5px
            }

            .form__multiple__input input:focus {
                width: 100%;
                background: rgba(255,255,255,0.5);
                border-bottom: 2px solid #f5a93d;
                ...
                .form__multiple__input input:focus::-webkit-input-placeholder {
                    color: #757575
                }

                .form__multiple__input input:focus::placeholder {
                    color: #757575
                }
    ...
    .form__option {
        ...
        .form__option a {
            ...
            .form__option a:hover {
                color: #3b74a5
            }

        .form__option .text-cancel {
            position: absolute;
            right: 40px;
            top: 3px;
            color: #8c8c8c
            ...
            .form__option .text-cancel:hover {
                color: #676767
            }

        .form__option .text-edit {
            position: absolute;
            right: 0px;
            top: 3px
        }
...
<body style=background:#000000>
    ...
    <nav style=background:#000000 class="mainmenu on_top">
        <div style=background:#000000 class=container-player>
            ...
            <ul>
                ...
                <li style=background:#000000><a href="常見問題new.php">常見問題</a></li>
    ...
    <div class=wrapper style=background:#000000>
                ...
                <form class= form method="post" action="">
                    <div class=form__wrap>
                        ...
                        <div class=form__multiple>
                            <label for>暱稱</label>
                            <div class=form__multiple__input>
                                <input  class=is-text type=text data-nick=小紅 value="<? echo $Little?>" maxlength=20 placeholder=來個新名字吧(ゝ∀･) name="Little" id="Little">
                            </div>
                            <div class="form__status is-warning" style=display:none>
                                十個中文字內，請避免使用特殊符號。
                            </div>
                            <div class=form__option style=display:none>
                            </div>
                        </div>
                        <div class=form__multiple>
                            <label for>信箱</label>
                            <div class=form__multiple__input>
                                <input type=text value="<? echo $mail?>" disabled>
                            </div>
                            <div id=add-email-text class="form__status is-warning" style=display:none>
                                系統將會發送訊息至您的信箱。<br>認證成功的信箱將無法註冊新帳號。
                            </div>
                            <div id=add-email-options class=form__option style=display:none>
                                
                            </div>
                            <hr class=line>
                            <div class=form__text>
                                <label for="Name">姓名</label>
                                <input  class=is-text type="text"  value="<? echo $Name?> "maxlength=20 placeholder=來個新名字吧(ゝ∀･) name="user" id="user">
                            </div>
                            <div class=form__text>
                                <label for="Phone">手機</label>
                                <input value="<? echo $Phone ?> " maxlength=20 name="P" id="P">
                            </div>
                            <div class=form__text>
                                <label for="Date">生日</label>
                                <input  type="date" value="<? echo $Date ?> "maxlength=20 placeholder=來個新名字吧(ゝ∀･) name="D" id="D">
                            </div>
                            <input type="submit" name="send" id="send"value="更新記錄">
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div style=position:fixed;left:20px;right:20px;bottom:20px;z-index:99>
            <div style="position:relative;padding:15px 30px 15px 15px;border:1px solid transparent;border-radius:3px;line-height:1.5;background:#FFFFDB;border-color:#E1D3B8;color:#705930;max-width:1250px;display:none">
                <a href=https://user.gamer.com.tw/editProfile_ssl.php class="alert-close oldbrowser-close" style="position:absolute;top:5px;right:5px;padding:3px;border:1px solid transparent;border-radius:2px;color:rgba(102,102,102,0.6)"></a>
                <p style=color:#705930>基於日前微軟官方表示 Internet Explorer 不再支援新的網路標準，可能無法使用新的應用程式來呈現網站內容，在瀏覽器支援度及網站安全性的雙重考量下，為了讓巴友們有更好的使用體驗，巴哈姆特即將於 2019年9月2日 停止支援 Internet Explorer 瀏覽器的頁面呈現和功能。屆時建議您使用下述瀏覽器來瀏覽巴哈姆特：</p>
            </div>
        </div>
    </div>
